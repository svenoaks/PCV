...
import smp.javax.sip.Dialog;
import smp.javax.sip.DialogState;
import smp.javax.sip.DialogTerminatedEvent;
import smp.javax.sip.ObjectInUseException;
import smp.javax.sip.SipException;
import smp.javax.sip.Timeout;
import smp.javax.sip.TimeoutEvent;
import smp.javax.sip.TransactionState;
import smp.javax.sip.address.Hop;
import smp.javax.sip.header.ContactHeader;
import smp.javax.sip.header.ExpiresHeader;
import smp.javax.sip.header.RSeqHeader;
...
public class SIPServerTransaction extends SIPTransaction implements ServerRequestInterface,
        smp.javax.sip.ServerTransaction, ServerTransactionExt {
    ...
    class SendTrying extends SIPStackTimerTask {
        ...
        protected SendTrying() {
            if (sipStack.isLoggingEnabled())
                sipStack.logWriter.logDebug("scheduled timer for " + SIPServerTransaction.this);
        ...
        protected void runTask() {
            ...
            if (realState == null || TransactionState.TRYING == realState) {
                if (sipStack.isLoggingEnabled())
                    sipStack.logWriter.logDebug(" sending Trying current state = "
    ...
    class TransactionTimer extends SIPStackTimerTask {
        ...
        public TransactionTimer() {
            if (sipStack.logWriter.isLoggingEnabled()) {
                sipStack.logWriter.logDebug("TransactionTimer() : " + getTransactionId());
            }
    ...
    protected SIPServerTransaction(SIPTransactionStack sipStack, MessageChannel newChannelToUse) {
        ...
        if (sipStack.isLoggingEnabled()) {
            sipStack.logWriter.logDebug("Creating Server Transaction" + this.getBranchId());
    ...
    public void processRequest(SIPRequest transactionRequest, MessageChannel sourceChannel) {
        ...
        if (sipStack.logWriter.isLoggingEnabled()) {
            sipStack.logWriter.logDebug("processRequest: " + transactionRequest.getFirstLine());
        ...
        try {
            ...
            if (getRealState() == null) {
                ...
                if (sipStack.isNon2XXAckPassedToListener()) {
                    ...
                    // CONFIRMED state
                    if (sipStack.logWriter.isLoggingEnabled()) {
                        sipStack.logWriter.logDebug("ACK received for server Tx "
                ...
                sipStack.logWriter.logDebug("completed processing retransmitted request : "
            ...
            if (TransactionState.COMPLETED != getRealState()
                ...
                if (((SIPTransactionStack) getSIPStack()).isDialogCreated(getOriginalRequest()
                    ...
                    if (sipStack.isLoggingEnabled())
                        sipStack.logWriter.logDebug("Too late to cancel Transaction");
                    this.semRelease();
                ...
                sipStack.logWriter.logDebug("Dropping request " + getRealState());
            }
    ...
    protected void fireRetransmissionTimer() {
        ...
        try {
            if (sipStack.getLogWriter().isLoggingEnabled()) {
                sipStack.getLogWriter().logDebug("fireRetransmissionTimer() -- ");
            }
    ...
    protected void fireTimeoutTimer() {
        ...
        if (sipStack.isLoggingEnabled())
            sipStack.logWriter.logDebug("SIPServerTransaction.fireTimeoutTimer this = " + this
